				...
				if (pb_graph_pins[i][j].port->type == OUT_PORT) {
					for (k = 0;
						...
						for (m = 0;
							...
							fprintf(fp, "pb_graph_node %s[%d].%s[%d].%s[%d] \n",
									pb_graph_pins[i][j].list_of_connectable_input_pin_ptrs[k][m]->parent_node->parent_pb_graph_node->parent_pb_graph_node->pb_type->name,
									pb_graph_pins[i][j].list_of_connectable_input_pin_ptrs[k][m]->parent_node->parent_pb_graph_node->parent_pb_graph_node->placement_index,
			...
			for (k = 0; k < pb_graph_pins[i][j].num_output_edges; k++) {
				...
				for (m = 0;
					...
					fprintf(fp,
							...
							pb_graph_pins[i][j].output_edges[k]->input_pins[m]->parent_node->pb_type->name,
							pb_graph_pins[i][j].output_edges[k]->input_pins[m]->parent_node->placement_index);
				}
				print_tabs(fp, level + 3);
